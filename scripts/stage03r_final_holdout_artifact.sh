...
CMD=("$PYTHON_CMD" -m src.evaluation.final_holdout_artifact)
if [[ -f data/db/a_share_hmm.duckdb ]]; then
  CMD+=(--db data/db/a_share_hmm.duckdb)
fi
OUTPUT_MD="reports/stage03r/final_holdout_artifact.md"
SUMMARY_JSON="reports/stage03r/final_holdout_artifact.json"
TMP_OUTPUT_DIR=""
if [[ ! -f data/db/a_share_hmm.duckdb ]]; then
  TMP_OUTPUT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/stage03r_final_holdout.XXXXXX")"
  OUTPUT_MD="$TMP_OUTPUT_DIR/final_holdout_artifact.md"
  SUMMARY_JSON="$TMP_OUTPUT_DIR/final_holdout_artifact.json"
