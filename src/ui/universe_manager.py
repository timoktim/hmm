...
from src.features.custom_basket_features import custom_basket_quality_frame
...
def _custom_basket_status(storage: DuckDBStorage, items: pd.DataFrame) -> pd.DataFrame:
    ...
    for item in custom_items.itertuples(index=False):
        ...
        basket = storage.read_df("SELECT basket_name, index_method FROM custom_stock_basket WHERE basket_id = ?", [basket_id])
        ...
        rows.append(
            {
                ...
                "最近有效成员数": "" if quality.empty else int(quality.iloc[-1]["member_count"]),
...
def render_universe_manager(storage: DuckDBStorage) -> None:
    ...
    if items.empty:
        ...
        if not basket_status.empty:
            st.caption("自定义股票池状态；等权模式按成员平均收益计算，自定义权重模式按成员权重归一化计算。")
    ...
    st.caption("等权模式会忽略权重；自定义权重模式可在代码后写权重，例如：300308 2.0 中际旭创。")
    ...
    if st.button("保存自定义股票池", disabled=not basket_name.strip()):
        ...
        if not members:
            ...
            basket_id = storage.create_custom_stock_basket(basket_name.strip(), basket_description.strip(), index_method=index_method)
