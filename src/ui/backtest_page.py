...
def render_backtest(storage: DuckDBStorage, universe_id: str | None = None) -> None:
    ...
    scope_col, threshold_col = st.columns([1, 2])
    scope = scope_col.radio("回测范围", scope_options, horizontal=True, format_func=lambda x: "全市场" if x == "all" else "当前板块池")
    threshold = threshold_col.slider("趋势状态置信度阈值", 0.30, 0.90, 0.55, 0.05, help=HELP_TEXTS["trend_up_threshold"])
    ...
    top_n = basic_cols[0].number_input("持有板块数量", min_value=1, max_value=20, value=5, help=HELP_TEXTS["top_n"])
    rebalance_days = basic_cols[1].number_input("调仓间隔（交易日）", min_value=1, max_value=30, value=5, help=HELP_TEXTS["rebalance_every"])
    n_states = basic_cols[2].number_input("HMM 隐藏状态数", min_value=2, max_value=6, value=3, help="板块模型将每个板块划分为几个隐藏状态。默认 3 个状态：趋势上行、中性震荡、风险回避。")
    train_window_days = basic_cols[3].number_input("滚动训练窗口（交易日）", min_value=60, max_value=1500, value=504, help=HELP_TEXTS["train_window"])
    ...
    start_date = date_cols[0].text_input("回测开始日期", value=default_start)
    end_date = date_cols[1].text_input("回测结束日期", value=default_end)
    st.subheader("高级参数")
    advanced_cols = st.columns([1, 1.7, 1, 1])
    retrain_frequency = advanced_cols[0].selectbox("HMM 重训频率", ["monthly", "quarterly", "signal"], index=0, format_func=lambda x: {"monthly": "每月", "quarterly": "每季度", "signal": "每个调仓信号日（最慢）"}[x])
    state_mode = advanced_cols[1].selectbox("状态来源", ["causal_backtest", "in_sample_display"], index=0, format_func=lambda x: "因果滚动训练（用于策略回测）" if x == "causal_backtest" else "训练样本内状态（非因果演示）")
    execution_price = advanced_cols[2].selectbox("执行价格", ["open", "close"], index=0, format_func=lambda x: "次日开盘" if x == "open" else "次日收盘")
    transaction_cost = advanced_cols[3].number_input("单边交易成本", min_value=0.0, max_value=0.01, value=0.001, step=0.0005, format="%.4f", help=HELP_TEXTS["transaction_cost"])
