...
def collect_environment(working_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "package_list_note": "omitted because dependency file hashes are recorded"
        if requirement_files
        else "recorded because no dependency files were found",
...
def collect_v0_fact_checks(connection: Any) -> dict[str, Any]:
    ...
    if table_exists(connection, "hsmm_lifecycle_ui_daily"):
        columns = table_columns(connection, "hsmm_lifecycle_ui_daily")
        sector_column = first_present(columns, SECTOR_COLUMNS)
        raw_score_columns = [column for column in columns if column.startswith("raw_score_used_")]
        lifecycle_filter = (
            "run_id = 'hsmm_lifecycle_primary_v1' "
            "and profile_mode = 'latest_asof' "
            "and state_date_policy = 'full_run' "
            "and profile_cutoff_date = ("
            "select max(profile_cutoff_date) from hsmm_lifecycle_ui_daily "
        ...
        checks["hsmm_lifecycle_ui_daily"] = {
            ...
            "sector_count_for_run": safe_scalar(
                connection,
                f"select count(distinct {quote_identifier(sector_column)}) from hsmm_lifecycle_ui_daily "
                f"where {lifecycle_filter}",
            )
            if sector_column
            else {"error": "no sector column found"},
            ...
            "duplicate_sector_date_keys": safe_scalar(
                connection,
                "select count(*) from ("
                f"select {quote_identifier(sector_column)}, trade_date, count(*) as n from hsmm_lifecycle_ui_daily "
                f"where {lifecycle_filter} "
                f"group by {quote_identifier(sector_column)}, trade_date having count(*) > 1"
                ")",
            )
            if sector_column
            else {"error": "no sector column found"},
            "raw_score_used_violation_count": collect_raw_score_used_violations(
                connection,
                raw_score_columns,
                columns,
                lifecycle_filter,
...
def collect_raw_score_used_violations(
    connection: Any,
    ...
    if not raw_score_columns:
        return {"error": "no raw_score_used columns found"}
    terms: list[str] = []
    for raw_column in raw_score_columns:
        suffix = raw_column.removeprefix("raw_score_used_")
        status_column = f"probability_status_{suffix}"
        if status_column in columns:
            terms.append(
                "("
                f"{quote_identifier(raw_column)} = true and "
                f"{quote_identifier(status_column)} in ('invalid', 'missing', 'insufficient_sample')"
                ")"
            )
    return safe_scalar(
        connection,
        "select count(*) from hsmm_lifecycle_ui_daily "
        f"where {lifecycle_filter} "
        f"and ({' or '.join(terms)})",
    ) if terms else {"error": "no matching probability status columns found"}
