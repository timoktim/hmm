# STAGE03V_WP2.1_full_target_streaming_audit
...
Codex instruction: `docs/codex_instructions/stage03v/CODEX_STAGE03V_WP2.1_full_target_streaming_audit.md`

Date: 2026-06-10

## Objective

Add a full-target streaming / blockwise audit between WP2 and WP3. WP2 validated target controls with synthetic tests and a 500-row audit sample. WP2.1 must now validate the complete WP1 Stage03V1 target row universe, without committing the full target dataset and without opening any baseline or model package.

This package must verify that the complete target universe is internally consistent across all entities, horizons, thresholds, target usages, split roles, target windows, labels, censoring states, purge/embargo metadata dependencies, and feature/target namespace policy prerequisites.

WP2.1 is an audit package only. It must not train models, calibrate probabilities, assign readiness, consume prospective holdout performance, alter target labels, or implement Stage03V2 / Stage03V3.
...
If these preconditions are not met, emit `blocked_wp2_not_ready` and stop.
...
- Rebuild or stream WP1 target rows by entity, slice, and/or date block.
- Validate the full target row universe against WP1 support totals and WP2 controls.
- Emit aggregate machine-readable audit reports and small capped error/audit samples.
- Add synthetic tests for chunked streaming behavior, count aggregation, and violation detection.
- Add a gate script.

Forbidden:

- Do not fetch external data.
- Do not commit full target dataset extracts.
- Do not write or modify DuckDB target tables.
- Do not mutate WP1 target labels or support reports.
...
The CSV artifacts must be small audit artifacts, not full data dumps:
...
- CI unit tests must not require private DuckDB; synthetic streaming tests are acceptance-critical.

## Full-target audit requirements

The audit must cover every WP1 target row that would be built from the accepted V7 source and accepted slices.
...
The full audit must report and validate:
...
For every streamed chunk, aggregate violation counts for:
...
By default, all counts above must be zero for pass. If numerical tolerance is needed for floating point recomputation, use explicit tolerances and report:
...
Default tolerance should be strict but realistic, e.g. `1e-10` for deterministic float recomputation unless implementation requires a documented alternative.

## Cross-cutoff and holdout requirements

The full audit must enforce:
...
Aggregate by `horizon`, `threshold_type`, `threshold_value`, `target_kind`, and `target_usage`. Compare to WP1 `slice_support_summary`.

Report for each slice:
...
## Purge / embargo dependency audit

WP2.1 does not need to re-run every purge operation on full target rows if WP2 already did policy validation, but it must confirm the full-target audit outputs are compatible with the accepted purge/embargo policy:
...
The audit must verify that the target artifact columns are target namespace only and cannot be mistaken for features.
...
Since WP2.1 does not build model features, this is a metadata/column namespace audit, not a model feature audit.
...
Create `tests/test_stage03v_full_target_audit.py`.
...
Also run the missing-V7 negative check listed above.

## Acceptance criteria

WP2.1 passes if:

- WP1 and WP2 support statuses are pass.
- V7 / SW2021 L2 verification is enforced.
- Missing V7 DB produces blocked status and no fallback.
- Full target row count is checked and matches WP1 support exactly.
- Entity and slice counts match expected values.
- Every slice support count matches WP1 support summary exactly.
- All target label/window/censoring/recompute violation counts are zero.
- Cross-cutoff and holdout checks pass.
- Purge/embargo input compatibility passes.
- Feature/target namespace audit passes.
- Chunk summary and error sample are small audit artifacts, not full data dumps.
- No model is trained.
- No probability is calibrated.
- No readiness is assigned.
- No holdout is consumed.
- No external data is fetched.
- No target DB table is written.
- No full target dataset is committed.
