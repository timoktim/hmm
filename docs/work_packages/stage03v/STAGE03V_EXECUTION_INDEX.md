...
Active package: STAGE03V-WP4-v1
...
WP0.5 completed sample-feasibility preflight on the V7 verified SW2021 L2 universe. WP1 completed the first formal Stage03V1 downside-risk target dataset builder and synthetic path-target tests. WP2 created the target-control gate for target leakage, permanent cross-cutoff censoring, purge/embargo, and feature/target namespace leakage policy. WP2.1 completed a full-target streaming / blockwise audit. WP3 completed causal baseline diagnostics. WP3.5 completed volatility-scaled threshold supplement and baseline metric sanity checks. WP4 is now active and implements logistic downside-risk hazard v1 on the accepted fixed-threshold Stage03V1 target family.
...
- `docs/codex_instructions/stage03v/CODEX_STAGE03V_WP4_logistic_downside_risk_hazard_v1.md`
...
| STAGE03V-WP4-v1 | Logistic Downside Risk Hazard v1 | active | stage03v/wp4-logistic-downside-risk-hazard-v1 | train deterministic logistic downside-risk hazard on fixed-threshold Stage03V1 targets |
| STAGE03V-WP5 | Calibration, Clustered Inference, and Downside Risk Readiness Matrix | blocked_until_wp4_accepted | pending | calibrate and assign readiness only after model validation artifacts exist |
...
1. Only STAGE03V-WP4-v1 is executable in the current Stage03V branch sequence.
2. STAGE03V-WP5 and later packages are blocked until WP4 is accepted.
3. WP4 must use the V7 DB path and accepted WP1 / WP2 / WP2.1 / WP3 / WP3.5 artifacts as hard inputs.
4. WP4 may train deterministic logistic hazard models on historical-development training folds only.
5. WP4 may evaluate on validation folds only.
6. WP4 must not calibrate probabilities.
7. WP4 must not assign `usable_probability`, `ordinal_only`, `baseline_only`, or any model readiness status.
8. WP4 must not fetch external data.
9. WP4 must not consume or inspect prospective final holdout performance.
10. WP4 must not modify WP1 target rows, labels, support reports, or target universe manifests.
11. WP4 must not replace the fixed-threshold Stage03V1 target mainline with volatility-scaled labels.
12. WP4 must not commit full target, feature, or score matrices.
13. WP4 must not write persistent DB tables by default.
14. WP4 must preserve WP2 purge/embargo and feature/target namespace controls.
15. WP4 must not modify HMM or HSMM training algorithms.
16. WP4 must not create UI, trading, buy/sell, recommendation, sizing, or decision outputs.
